import { Alert, SafeAreaView, StyleSheet, Text, View } from 'react-native'
import React, { useRef } from 'react'
import { Avatar, Button } from '@rneui/base'
import ImagePicker from '../components/ImagePicker';
import { RNS3 } from 'react-native-aws3';
import { useNavigation } from '@react-navigation/native';
import config from '../config/config';
import AsyncStorage from '@react-native-async-storage/async-storage';

const ScreenOne = () => {
    const rawimg = "data:image/png;base64,";
    const [isImage, setImage] = React.useState("");
    const imageref = useRef();
    const navigation = useNavigation();
    const fileupload = (image) => {
        setImage(image)
        console.log(image?.path)
        imageref.current.close();
    }




    const getimage = async () => {
        const myimg = await AsyncStorage.getItem('image');
        if (myimg) {
            setImage({
                path: myimg
            })
        }
    }
    React.useEffect(() => getimage(), [])
    const onSubmitform = () => {


        if (!isImage) {
            Alert.alert("error", "Please Upload Image");
            return false;
        }

        console.log(isImage);


        const imagext = isImage?.path.split('.');
        const file = {
            uri: isImage.sourceURL,
            name: Date.now()+ isImage.filename,
            type: isImage.mime
        }

        const options = {
            bucket: config.bucket,
            region: config.region,
            accessKey: config.accessKey,
            secretKey: config.secretKey,
            successActionStatus: 201
        }

        RNS3.put(file, options).then(async (response) => {
            if (response.status !== 201) {
                throw Alert.alert("Insufficient permission to access", "Failed to upload image to S3");
            } else {
                await AsyncStorage.setItem('image', response.body?.postResponse.location)
            }
        });
    }
    const gotonext = () => {
        if(!isImage){
            Alert.alert("error","oppo's please upload image");
            return false
        }
        navigation.navigate('screenTwo');
    }


    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.views}>
                <Avatar
                    rounded
                    size={150}
                    source={{
                        uri: isImage?.path || rawimg
                    }}
                >
                    <Avatar.Accessory onPress={() => imageref.current.open()} size={30} color={"#5D3FD3"} />
                </Avatar>

                <View style={styles.button}>
                    <Button title={"Submit"} onPress={onSubmitform} />
                    <Button title={"next"} onPress={gotonext} />
                </View>
            </View>

            <ImagePicker ref={imageref} onFileSelected={fileupload} />
        </SafeAreaView>

    )
}

export default ScreenOne

const styles = StyleSheet.create({
    container: {
        flex: 1
    },
    views: {
        display: 'flex',
        justifyContent: 'center',
        alignItems: 'center',
        flex: 1
    },
    button: {
        width: 200,
        marginTop: 80,
        display: 'flex',
        flexDirection: 'row',
        justifyContent: 'space-between'



    }
})